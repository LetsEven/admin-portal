...
import {
  useServicesOnboarding,
  joyrideTheme,
  joyrideResponsiveCSS,
} from "../hooks/useServicesOnboarding";
...
const SERVICE_MAPPING: Record<string, ServiceInfo> = {
  "tap-order-pay": {
    ...
    name: "Tap Order & Pay",
    description:
      "Órdenes y pagos sin mesero, desde la mesa. El cliente toca con su celular la tarjeta NFC de la mesa, accede al menú digital, ordena y paga desde su dispositivo. El pedido se procesa automáticamente y se entrega. Ideal para agilizar el servicio y reducir tiempos de espera.",
    icon: "smartphone",
    enabled: false,
  },
  "flex-bill": {
    ...
    name: "Flex Bill",
    description:
      "Órdenes grupales con cuenta compartida y pagos divididos. Cada comensal hace tap en la tarjeta NFC para unirse a una cuenta compartida. Cada quien puede ordenar desde su celular y, al final, pagar solo lo que consumió o dividir la cuenta fácilmente. Perfecto para grupos y mesas grandes.",
    icon: "receipt",
    enabled: false,
  },
  "food-hall": {
    ...
    name: "Food Hall",
    description:
      "Órdenes unificadas en espacios con múltiples locales. Diseñado para food courts, parques gastronómicos o mercados: el usuario puede ordenar de varios locales en una sola transacción, pagar desde su celular y recibir notificaciones cuando su pedido esté listo para recoger.",
    icon: "layout-grid",
    enabled: false,
  },
  "tap-pay": {
    ...
    name: "Tap & Pay",
    description:
      "Pago moderno, sin cambiar la experiencia tradicional. El cliente pide con el mesero como siempre, pero al pagar solo necesita acercar su celular a la tarjeta NFC para liquidar. También puede dividir la cuenta sin usar terminal ni esperar al mesero. Rápido, elegante y sin fricción.",
    icon: "scan-line",
    enabled: false,
  },
  "pick-n-go": {
    ...
    name: "Pick N Go",
    description:
      "Órdenes anticipadas desde redes sociales o sitio web. El cliente ordena y paga desde su celular antes de llegar al local, a través de un link el cual puede ser publicado en redes sociales. Al llegar, simplemente recoge su pedido ya listo.",
    icon: "shopping-cart",
    enabled: false,
  },
  "room-service": {
...
const PromotionsManagement = () => {
  ...
  const [showInactiveServiceModal, setShowInactiveServiceModal] =
    useState(false);
  ...
  const getIcon = (iconName: any) => {
    switch (iconName) {
      case "smartphone":
        return (
          <SmartphoneIcon className="h-7 w-7 text-custom-green-600 group-hover:text-custom-green-600 transition-colors duration-200" />
        );
      case "receipt":
        return (
          <ReceiptIcon className="h-7 w-7 text-custom-green-600 group-hover:text-custom-green-600 transition-colors duration-200" />
        );
      case "layout-grid":
        return (
          <LayoutGridIcon className="h-7 w-7 text-custom-green-600 group-hover:text-custom-green-600 transition-colors duration-200" />
        );
      case "scan-line":
        return (
          <ScanLineIcon className="h-7 w-7 text-custom-green-600 group-hover:text-custom-green-600 transition-colors duration-200" />
        );
      case "shopping-cart":
        return (
          <ShoppingCartIcon className="h-7 w-7 text-custom-green-600 group-hover:text-custom-green-600 transition-colors duration-200" />
        );
      case "bed":
        return (
          <BedIcon className="h-7 w-7 text-custom-green-600 group-hover:text-custom-green-600 transition-colors duration-200" />
        );
      default:
        return (
          <ShoppingBagIcon className="h-7 w-7 text-custom-green-600 group-hover:text-custom-green-600 transition-colors duration-200" />
        );
    }
  };
  return (
    <div className="w-full">
            ...
            return (
              ...
              >
                <div className="p-4 flex">
                  <div
                    className={`flex-shrink-0 flex items-center justify-center w-14 h-14 rounded-full transition-colors duration-200 ${
                      isEnabled
      ...
      {/* Dashboard Modals */}
      <TapOrderDashboardModal
        isOpen={showTapOrderDashboard}
        onClose={() => setShowTapOrderDashboard(false)}
      />
      <FlexBillDashboardModal
        isOpen={showFlexBillDashboard}
        onClose={() => setShowFlexBillDashboard(false)}
      />
      <FoodHallDashboardModal
        isOpen={showFoodHallDashboard}
        onClose={() => setShowFoodHallDashboard(false)}
      />
      <PickNGoDashboardModal
        isOpen={showPickNGoDashboard}
        onClose={() => setShowPickNGoDashboard(false)}
      />
      <TapPayDashboardModal
        isOpen={showTapPayDashboard}
        onClose={() => setShowTapPayDashboard(false)}
      />
