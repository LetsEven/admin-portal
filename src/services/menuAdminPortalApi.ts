...
export interface CreateSectionRequest {
  ...
  clasificacion?: number; // 1: ALIMENTOS, 2: BEBIDAS, 3: OTROS
}
...
class MenuAdminPortalApiService {
  ...
  async getAllItems(
    token: string,
    filters: { section_id?: number; is_available?: boolean } = {},
  ): Promise<MenuItem[]> {
    ...
    return this.makeRequest<MenuItem[]>(
      endpoint,
      {
        method: "GET",
      },
  ...
  async getAllItemsByBranch(
    branchId: string | null,
    token: string,
  ): Promise<MenuItem[]> {
    const endpoint = branchId
      ? `/items/by-branch/${branchId}`
      : "/items/by-branch";
    return this.makeRequest<MenuItem[]>(
      endpoint,
      {
...
export function useMenuAdminPortalApi() {
  ...
  return {
    ...
    sections: {
      getAll: () =>
        makeAuthenticatedRequest((token) =>
          menuAdminPortalApiService.getAllSections(token),
        ),
      create: (data: CreateSectionRequest) =>
        makeAuthenticatedRequest((token) =>
          menuAdminPortalApiService.createSection(data, token),
        ),
      update: (id: number, data: UpdateSectionRequest) =>
        makeAuthenticatedRequest((token) =>
          menuAdminPortalApiService.updateSection(id, data, token),
        ),
      delete: (id: number) =>
        makeAuthenticatedRequest((token) =>
          menuAdminPortalApiService.deleteSection(id, token),
        ),
      reorder: (sections: { id: number; display_order: number }[]) =>
        makeAuthenticatedRequest((token) =>
          menuAdminPortalApiService.reorderSections(sections, token),
        ),
    },

    // Operaciones de items
    items: {
      getAll: (filters?: { section_id?: number; is_available?: boolean }) =>
        makeAuthenticatedRequest((token) =>
          menuAdminPortalApiService.getAllItems(token, filters),
        ),
      getAllByBranch: (branchId: string | null) =>
        makeAuthenticatedRequest((token) =>
          menuAdminPortalApiService.getAllItemsByBranch(branchId, token),
        ),
      getById: (id: number) =>
        makeAuthenticatedRequest((token) =>
          menuAdminPortalApiService.getItemById(id, token),
        ),
      create: (data: CreateItemRequest) =>
        makeAuthenticatedRequest((token) =>
          menuAdminPortalApiService.createItem(data, token),
        ),
      update: (id: number, data: UpdateItemRequest) =>
        makeAuthenticatedRequest((token) =>
          menuAdminPortalApiService.updateItem(id, data, token),
        ),
      delete: (id: number) =>
        makeAuthenticatedRequest((token) =>
    ...
    menu: {
      getComplete: () =>
        makeAuthenticatedRequest((token) =>
          menuAdminPortalApiService.getCompleteMenu(token),
        ),
    },
  };
}
