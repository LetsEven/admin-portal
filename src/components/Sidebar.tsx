...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  return <div className={`h-0 flex-1 flex flex-col overflow-y-auto transition-all duration-300 ease-in-out ${!mobile && 'group'}`} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      {/* Logo */}
      <div className={`flex items-center justify-center h-24 flex-shrink-0 px-4 bg-white shadow-md rounded-lg mt-3 ${mobile || isExpanded ? 'mx-3' : 'ml-1 mr-0'}`}>
        <div className="relative w-full h-full flex items-center justify-center">
          {/* Full logo for expanded state */}
          <img src="/logo-green.webp" alt="Xquisito Logo" className={`h-8 w-auto max-w-full transition-opacity ${mobile || isExpanded ? 'duration-[800ms]' : 'duration-0'} ease-out ${mobile || isExpanded ? 'opacity-100' : 'opacity-0 absolute'}`} />
          {/* Compact logo for collapsed state */}
          <div className={`flex items-center justify-center transition-opacity ${mobile || isExpanded ? 'duration-0' : 'duration-[800ms]'} ease-out ${mobile || isExpanded ? 'opacity-0 absolute' : 'opacity-100'}`}>
            <img src="/logo-short-green.webp" alt="Xquisito Logo" className="h-8 w-auto object-contain" />
          </div>
        </div>
      </div>
      {/* Navigation */}
      <nav className={`mt-3 flex-1 py-4 bg-white space-y-2 rounded-lg shadow-md ${mobile || isExpanded ? 'mx-3 px-6' : 'ml-1 mr-0 px-3'}`}>
        <Link href="/" className={`group flex items-center text-sm font-medium rounded-lg transition-all duration-200 ${mobile || isExpanded ? 'px-3 py-3' : 'px-0 py-3 justify-center'} ${pathname === '/' ? 'bg-custom-green-100 text-custom-green-900 shadow-sm' : 'text-gray-700 hover:bg-gray-100 hover:text-gray-900'}`} data-tour="home-dashboard">
        ...
        <Link href="/promotions" className={`group flex items-center text-sm font-medium rounded-lg transition-all duration-200 ${mobile || isExpanded ? 'px-3 py-3' : 'px-0 py-3 justify-center'} ${pathname === '/promotions' ? 'bg-custom-green-100 text-custom-green-900 shadow-sm' : 'text-gray-700 hover:bg-gray-100 hover:text-gray-900'}`}>
        ...
        <Link href="/rewards" className={`group flex items-center text-sm font-medium rounded-lg transition-all duration-200 ${mobile || isExpanded ? 'px-3 py-3' : 'px-0 py-3 justify-center'} ${pathname === '/rewards' ? 'bg-custom-green-100 text-custom-green-900 shadow-sm' : 'text-gray-700 hover:bg-gray-100 hover:text-gray-900'}`}>
        ...
        <Link href="/settings" className={`group flex items-center text-sm font-medium rounded-lg transition-all duration-200 ${mobile || isExpanded ? 'px-3 py-3' : 'px-0 py-3 justify-center'} ${pathname === '/settings' ? 'bg-custom-green-100 text-custom-green-900 shadow-sm' : 'text-gray-700 hover:bg-gray-100 hover:text-gray-900'}`}>
