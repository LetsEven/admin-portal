    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center">
      ...
      <div className="relative w-full max-w-6xl h-[90vh] mx-auto bg-white rounded-2xl shadow-xl overflow-hidden animate-in fade-in duration-200 ease-out">
        ...
        <div className="flex items-center justify-between p-5 border-b border-gray-200">
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className="p-2 rounded-full text-gray-400 hover:text-gray-500 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#0EA5E9]"
              disabled={isLoading}
            >
              <RefreshCwIcon
          ...
          {loadingStats ? (
            <div className="flex items-center justify-center h-full">
              <div className="text-center">
                <RefreshCwIcon className="h-12 w-12 text-custom-green-600 animate-spin mx-auto mb-4" />
              ...
              <div className="flex justify-end mb-6">
                <div className="inline-flex rounded-md shadow-sm">
                  <button
                    ...
                    className={`px-4 py-2 text-sm font-medium rounded-l-md ${timeRange === "daily" ? "bg-custom-green-600 text-white" : "bg-white text-gray-700 hover:bg-gray-50"} border border-gray-300 focus:z-10 focus:outline-none focus:ring-1 focus:ring-custom-green-500`}
                  >
                    Diario
                  </button>
                  <button
                    onClick={() => setTimeRange("weekly")}
                    className={`px-4 py-2 text-sm font-medium ${timeRange === "weekly" ? "bg-custom-green-600 text-white" : "bg-white text-gray-700 hover:bg-gray-50"} border-t border-b border-gray-300 focus:z-10 focus:outline-none focus:ring-1 focus:ring-custom-green-500`}
                  >
                    Semanal
                  </button>
                  <button
                    onClick={() => setTimeRange("monthly")}
                    className={`px-4 py-2 text-sm font-medium rounded-r-md ${timeRange === "monthly" ? "bg-custom-green-600 text-white" : "bg-white text-gray-700 hover:bg-gray-50"} border border-gray-300 focus:z-10 focus:outline-none focus:ring-1 focus:ring-custom-green-500`}
                  >
                    Mensual
                  </button>
                </div>
              ...
              <div className="bg-white p-5 rounded-lg shadow border border-gray-100 mb-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  ...
                  <div className="p-4 bg-gray-50 rounded-lg">
                    ...
                    <p className="text-2xl font-semibold text-gray-900">{totalDelivered}</p>
                  </div>
                  <div className="p-4 bg-gray-50 rounded-lg">
                    <p className="text-sm text-gray-500">Abiertos</p>
                    <p className="text-2xl font-semibold text-gray-900">{totalOpened}</p>
                  </div>
                  <div className="p-4 bg-gray-50 rounded-lg">
                    <p className="text-sm text-gray-500">Clicks</p>
                    <p className="text-2xl font-semibold text-gray-900">{totalClicked}</p>
                  </div>
                  <div className="p-4 bg-gray-50 rounded-lg">
                    <p className="text-sm text-gray-500">Canjeados</p>
                    <p className="text-2xl font-semibold text-gray-900">{totalRedeemed}</p>
                  </div>
                  <div className="p-4 bg-gray-50 rounded-lg">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      {campaign?.templates && campaign.templates.length > 0 ? (
                        ...
                        <p className="mt-1 text-sm text-gray-500">Sin templates asociados</p>
                      )}
                    </div>
                    <div>
                      <h4 className="text-sm font-medium text-gray-500">
                    ...
                    {campaign?.rewardValue && (
                      <div>
                        <h4 className="text-sm font-medium text-gray-500">Valor de recompensa</h4>
                        <p className="mt-1 text-base font-medium text-gray-900">
                  ...
                  <div className="space-y-3">
                    {Object.entries(stats?.by_delivery_method || {}).map(([method, count]) => (
                      <div key={method} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        <div className="flex items-center">
                          {method === 'email' && <MailIcon className="h-5 w-5 text-blue-600 mr-2" />}
                          {method === 'sms' && <MessageSquareIcon className="h-5 w-5 text-yellow-600 mr-2" />}
                          {method === 'whatsapp' && <MessageSquareIcon className="h-5 w-5 text-green-600 mr-2" />}
                    ...
                    {Object.keys(stats?.by_delivery_method || {}).length === 0 && (
                      <p className="text-sm text-gray-500 text-center py-4">No hay datos de canales disponibles</p>
                    )}
                  </div>
                </div>
              </div>
        ...
        <div className="border-t border-gray-200 p-4 flex justify-end">
          <button
            onClick={handleExportData}
            className="mr-3 inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-custom-green-500"
            disabled={!stats}
          >
          ...
          <button
            onClick={onClose}
            className="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-custom-green-600 hover:bg-custom-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-custom-green-500"
          >
            Cerrar
          </button>
        </div>
