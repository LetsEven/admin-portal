...
const RestaurantHeader: React.FC<RestaurantHeaderProps> = ({
  ...
  const isNewRestaurant =
    restaurantName === "Mi Restaurante" && !bannerImage && !logoImage;
  ...
  const loadBranches = async () => {
    ...
    try {
      ...
      const normalized = (branchesData.branches || []).map((b) => ({
        ...b,
        address: b.address ?? "",
      }));
      ...
      if (!selectedBranch && normalized.length > 0) {
        onBranchChange?.(normalized[0]);
      }
    } catch (error) {
  ...
  const handleCropSave = async (croppedImage: string) => {
    ...
    try {
      ...
      const file = new File([blob], `logo_${user.id}.jpg`, {
        type: "image/jpeg",
      });
  ...
  const handleBannerCropSave = async (croppedImage: string) => {
    ...
    try {
      ...
      const response = await fetch(croppedImage);
      const blob = await response.blob();
      const file = new File([blob], `banner_${user.id}.jpg`, {
        type: "image/jpeg",
      });
          ...
          <div className="flex items-start space-x-3 sm:space-x-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-blue-700 text-xs sm:text-sm mb-3 sm:mb-4">
                Personaliza la información de tu restaurante para crear una
                experiencia única. Comienza editando el nombre haciendo clic en
                "Mi Restaurante" más abajo.
              </p>
                    ...
                    onClick={async () => {
                      if (logoImage) {
                        try {
                          ...
                          const imagePromise = new Promise<string>(
                            (resolve, reject) => {
            ...
            {branches.length > 0 && (
              <div className="flex items-center space-x-2 sm:mr-3">
                <button
                  ...
                  className={`inline-flex items-center px-2.5 sm:px-4 py-1.5 sm:py-2 border border-transparent rounded-md shadow-sm bg-custom-green-500 text-xs sm:text-sm font-medium text-white hover:bg-custom-green-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-custom-green-600 transition-all duration-200 w-full sm:w-auto justify-center ${
                    showBranchDropdown
                      ? "ring-2 ring-custom-green-500 ring-offset-2"
                      : ""
                  }`}
                >
                  ...
                  <ChevronDown
                    className={`ml-1.5 sm:ml-2 h-3.5 w-3.5 sm:h-4 sm:w-4 text-gray-300 transition-transform duration-200 ${
